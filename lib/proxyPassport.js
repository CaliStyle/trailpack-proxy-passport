...
module.exports = {
  init: (app) => {
    ...
    passport.connect = function (req, query, profile, next) {
      const Passport = app.orm['Passport']
      const user = {}
        ...
        .then(passport => {
          if (!req.user) {
            ...
            if (!passport) {
              ...
              app.config.proxyPassport.mergeThirdPartyProfile(user, profile)
                .then(mergedProfile => app.orm['User'].create(mergedProfile)
                  .then(user => {
                    query.tokens = JSON.stringify(query.tokens)
                    return user.createPassport(query)
            ...
            // Action:   Get the user associated with the passport.
              ...
              return Promise.all([
                ...
                Passport.update(passport.id, passport)
              ])
                .then(results => {
  ...
  loadStrategies: (app) => {
    const passport = app.services.PassportService.passport
    _.each(app.config.proxyPassport.strategies, (strategiesOptions, name) => {
  ...
  addRoutes: app => {
    const prefix = _.get(app.config, 'proxyPassport.prefix') || _.get(app.config, 'footprints.prefix')
   ...
   */
  copyDefaults: (app) => {
    app.config.proxyPassportDefaults = _.clone(app.config.proxyPassport)
    return Promise.resolve({})
  },
  /**
